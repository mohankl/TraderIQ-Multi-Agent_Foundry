...
OTel package pinning is **load-bearing**. `azure-monitor-opentelemetry-exporter <= 1.0.0b45` imports `LogData` from `opentelemetry.sdk._logs`, but that symbol was removed in sdk 1.41. The compatible set is `azure-monitor-opentelemetry >= 1.8.8` + `azure-monitor-opentelemetry-exporter >= 1.0.0b52` + `opentelemetry-sdk < 1.41`. `pyproject.toml` pins to that set; don't relax those constraints without testing.
