...
| `finbot-web` | `alphastatetradingacr.azurecr.io/finbot-web:v12` | 3000 | Next.js 16 frontend; `/api/chat` proxies SSE; renders interleaved step pills, text deltas, and inline cards progressively. Each `TEXT_MESSAGE_START` opens a new text segment so successive message items render as distinct markdown blocks. Shows a "Thinking…" label before any event arrives and a "Writing analysis" hint between tool-done and first token. Branded as **Trading IQ** with a custom network-with-dollar SVG logo |
