...
1. **Bump the agent version.** Update `AZURE_EXISTING_AGENT_VERSION` env in the `tradingiq-api` Container App. Confirm with the user before applying.
2. **Add/remove MCP tools.** Edit `mcp-server/server.py`, add the new `@mcp.tool(...)` function, rebuild and redeploy `tradingiq-mcp` (delegate to `deployer`).
